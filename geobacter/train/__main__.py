...
def main():
    ...
    train_dataset = OsmTileDataset(
        samples=[sample for sample in load_samples(Path("data/extents/train_100000.json"))
    ...
    triplet_loss = TripletLoss(margin=1.5)
    ...
    for idx in range(10):
        anchor = DENORMALIZE(anchors[idx, :].squeeze())
        positive = DENORMALIZE(positives[idx, :].squeeze())
        negative = DENORMALIZE(negatives[idx, :].squeeze())
    ...
    def test(engine):
            ...
            for sample in test_loader:
                ...
                for i in range(len(anchor_embedding)):
                    ...
                    if len(embeddings) < 300:
                        embeddings.append(
                            anchor_embedding[i, :].squeeze().detach().cpu().numpy()
                        )
                        images.append(
                            DENORMALIZE(anchor[i, :].squeeze()).detach().cpu().numpy().transpose(1, 2, 0)
                        )
                    else:
                        break
        ...
        plt.draw()
        plt.savefig(f"assets/embeddings/{engine.state.epoch}.png")
